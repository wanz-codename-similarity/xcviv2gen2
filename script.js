...
// ====== ATTACK FUNCTIONALITY (REPLACEMENT) ======
...
if (!/^\d+$/.test(cleanTarget) || cleanTarget.startsWith('0') || cleanTarget.length < 8 || cleanTarget.length > 15) {
  ...
  loading.classList.add('active');
  attackStatus.textContent = 'Serangan diproses';
  ...
  attackDetails.insertAdjacentHTML('afterend', `
    <div id="${statusEntryId}" class="attack-entry" style="margin-top:10px;">
      <div class="attack-entry-header"><strong>Proses:</strong> <span class="entry-status">Diproses</span></div>
      <div class="attack-entry-body">
        <div class="progress-bar" style="width:100%; background:rgba(255,255,255,0.06); height:8px; border-radius:6px; overflow:hidden; margin-top:8px;">
          <div class="progress-fill" style="width:0%; height:100%; background:linear-gradient(90deg,var(--primary),var(--secondary));"></div>
        </div>
        <div class="entry-log" style="font-size:12px; color:var(--text); margin-top:8px;">Menunggu hasil...</div>
      </div>
    </div>
  `);
  ...
  (async () => {
    try {
      ...
      entryLog.textContent = 'Mengirim permintaan ke server...';
      ...
      if (res && res.ok) {
        ...
        entryStatus.textContent = 'Sukses';
        entryLog.textContent = (res.message || 'Permintaan diproses — status: sukses.');
        showToast(`Serangan ${attackOption.toUpperCase()} dikirim (async).`, 'success');
      } else {
        progressFill.style.width = '100%';
        entryStatus.textContent = 'Gagal';
        const errText = (res && (res.error || res.raw && JSON.stringify(res.raw))) || 'Unknown error';
        entryLog.textContent = `Gagal: ${errText}`;
        showToast(`Serangan gagal: ${errText}`, 'error');
      }
    } catch (err) {
      progressFill.style.width = '100%';
      entryStatus.textContent = 'Gagal';
      entryLog.textContent = `Exception: ${err && err.message ? err.message : String(err)}`;
      console.error('[attack async] error', err);
      showToast('Server error saat memproses serangan', 'error');
    } finally {
      // Selesaikan loading visual
      loading.classList.remove('active');
      // reset form for UX
